...
# Compare models #### if p value is less than 0.5, then the data explains different things
...
mod3mse <- mean(residuals(mod3)^2) #this is best, has lowest value
...
# + means allow intercept to change
# * means allow the slope AND intercept to change
# which model is best?
...
# Using mtcars #
skim(mtcars)
carsmod1 <- lm(data=mtcars, formula = mpg ~ wt + factor(cyl)) #1
carsmod2 <- lm(data=mtcars, formula = mpg ~ wt * factor(cyl)) #2

#1 changes the intercept, the "est st.Dev" changes y axis
#2 changes both intercept and slope
...
set <- caret::createDataPartition(iris$Sepal.Width, p=.5) # pick random subset of data 
...
#cant have 1.5 cylinders so make it a factor first 
#am, vs, cyl need to be factors (as manuel, cylinders)
...
mod2 <- lm(data=mtcars, formula = mpg ~ cyl + hp)
mod3 <- lm(data=mtcars, formula = mpg ~ cyl * hp)
...
# Evaluate predictions ####
df_mod1 <- add_predictions(mtcars,mod1) # adds model prediction column using a single model
df_mod1
formula(mod1)
...
# compare predictions to reality
p1 <- ggplot(df_mod1,aes(x=cyl,color=hp)) +
  geom_point(aes(y=mpg),alpha=.5,size=2) +
  geom_point(aes(y=pred),color="Black") + theme_bw()
p1

p1 + geom_segment(aes(y=mpg,xend=cyl,yend=pred),
                  linetype=2,color="Black",alpha=.5)
